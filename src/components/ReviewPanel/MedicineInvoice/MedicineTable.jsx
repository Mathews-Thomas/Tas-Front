...
function MedicineTable({ data, loader, fetchData, setRefresh }) {
  ...
  const handleEditClick = async (invoice) => {
    ...
    if (canEdit) {
      ...
      Swal.fire({
        title: "Error!",
        text: "You are not allowed to edit this invoice.",
        icon: "error",
                ...
                <div className="flex justify-end mt-5 gap-4">
                  {!isCustomNumber && (
                    <>
                      <button
                        onClick={() =>
                          handleSendWhatsapp(selectedRow?.patientID?.phone)
                        }
                        id="PatientButton"
