...
const InvoicePage = ({ setRefreshList }) => {
  ...
  const fetchInvoiceData = useCallback(async () => {
    try {
      const response = await Axios.get(
        `/add-invoice?BranchID=${BranchID}&PatientID=${PatientID}`
      );
  ...
  }, [PatientID, BranchID, fetchInvoiceData]);
  ...
  const onSubmit = () => {
    ...
    // showAlert("Success", "Invoce Added", "success");
    Axios.post("/add-invoice", { ...formData, BranchID })
      .then(() => {
        setFormData(initialValue);
